...
public class RestResult<T extends RestResult<T>> {
    ...
    public static <T extends RestResult<T>> RestResult<T> success(Object data) {
        return (new RestResult.Builder<T>()).status(RestErrorStatus.SUCCESS.getStatus()).data(data).build();
    }
    ...
    public static <T extends RestResult<T>> RestResult<T> success(String message, Object data) {
        return (new RestResult.Builder<T>()).status(RestErrorStatus.SUCCESS.getStatus()).message(message).data(data).build();
    }

    public static <T extends RestResult<T>> RestResult<T> success(RestStatus status) {
        return (new RestResult.Builder<T>()).status(status.getStatus()).message(status.getMessage()).build();
    }

    public static <T extends RestResult<T>> RestResult<T> success(RestStatus status, Object data) {
        return (new RestResult.Builder<T>()).status(status.getStatus()).message(status.getMessage()).data(data).build();
    }

    public static <T extends RestResult<T>> RestResult<T> error(Integer status, String message, Object data) {
    ...
    public static <T extends RestResult<T>> RestResult<T> error(RestStatus status) {
        return (new RestResult.Builder<T>()).status(status.getStatus()).message(status.getMessage()).build();
    }

    public static <T extends RestResult<T>> RestResult<T> error(RestStatus status, Object data) {
        return (new RestResult.Builder<T>()).status(status.getStatus()).message(status.getMessage()).data(data).build();
    }
