...
import cyan.toolkit.rest.util.StreamUtils;
...
final class DefaultImageService implements ImageService {
    ...
    @Override
    public Boolean support(MultipartFile file) throws AuthException {
        return true;
    }

    @Override
    public ImageInfo upload(@NonNull MultipartFile file) throws AuthException {
        ...
        try {
            ...
            if (GeneralUtils.isEmpty(extension)) {
                throw new AuthException("图片格式错误！", new ParamInvalidException());
            }
            ...
            throw new AuthException("上传错误！error: ".concat(exception.getMessage()), new ResourceNotFoundException());
        } catch (IOException exception) {
            throw new AuthException("meta信息生成错误！error: ".concat(exception.getMessage()), new FileErrorException());
        }
        return imageInfo;
    }

    @Override
    public void download(@NonNull String uuid, @NonNull HttpServletResponse response) throws AuthException {
        try {
            ...
            throw new AuthException("下载错误！error: ".concat(exception.getMessage()), new ResourceNotFoundException());
        } catch (IOException exception) {
            throw new AuthException("下载错误！error: ".concat(exception.getMessage()), new StreamTransferException());
        }
    }

    @Override
    public ImageCapability capability() throws AuthException {
        return new ImageCapability();
    }

    @Override
    public ImageInfo info(String uuid) throws AuthException {
        try {
            String uuidPath = ImageManager.uuid(uuid);
            String metaPath = uuidPath.concat(File.separator).concat(uuid).concat(".meta");
            ...
            throw new AuthException("meta信息读取错误！error: ".concat(exception.getMessage()), new FileErrorException());
        }
    }

    @Override
    public void delete(String uuid) throws AuthException {
        throw new AuthException("不支持文件删除!");
    }
